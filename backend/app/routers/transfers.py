...
def transfer_room(
    ...
    old = (
        ...
        .filter(Occupancy.id == payload.old_occupancy_id)
        .first()
    )
    ...
    if not old:
        raise HTTPException(
            ...
            detail="Old occupancy not found",
        )

    if old.tenant_id != tenant.id:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Old occupancy does not belong to this tenant",
        )
    ...
    (
        ...
        .filter(
            RoomListing.room_id == new_room.id,
